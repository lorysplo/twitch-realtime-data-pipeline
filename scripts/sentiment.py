#!/usr/bin/env python3
"""
sentiment.py — Rule-based sentiment scoring (VADER + a Twitch emote lexicon).
Shared by both the realtime path (realtime.py) and the offline (batch) path, so the algorithm stays identical on both sides.
No ML: VADER handles the text, and the emote lexicon adds the emote-specific sentiment that is unique to Twitch (VADER does not recognize emotes).
"""
from vaderSentiment.vaderSentiment import SentimentIntensityAnalyzer

_analyzer = SentimentIntensityAnalyzer()

# Sentiment weights (-1 to +1) for high-frequency Twitch emotes, maintained manually
EMOTE_SENTIMENT = {
    # Positive / hyped
...
def extract_emotes(text, emotes_field):
    """Use the position indexes in the emotes field to pull the emote names out of text."""
    names = []
    if not emotes_field:
        return names
...
def score(text, emotes_field=""):
    """Return (compound score from -1 to +1, label pos/neu/neg)."""
    text = text or ""
    ...
    if emote_scores:
        ...
        comp = 0.5 * v + 0.5 * e          # text and emotes each count for half
    else:
        comp = v
